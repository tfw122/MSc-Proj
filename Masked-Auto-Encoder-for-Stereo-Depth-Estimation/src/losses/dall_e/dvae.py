...
from src.losses.dall_e import load_model
...
class Dalle_VAE(BasicVAE):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.dalle_config = self.config.model_config.dall_e
        self.image_size = self.config.dataset_config.preprocess.vision_transforms.params.Resize.size[0]
        # load the model;
        self.encoder, self.decoder = self.load_model()
        
    def load_model(self):
        encoder = load_model(os.path.join(self.dalle_config.model_dir, "encoder.pkl"))
        decoder = load_model(os.path.join(self.dalle_config.model_dir, "decoder.pkl"))
        print('DALL-E model loaded with original OpenAI pre-trained weights!')
