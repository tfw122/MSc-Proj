"""
Registry is central source of truth in this repository. Registry maintains mappings of various information
...
- Register a dataset: ``@registry.register_datamodule``
...
- Register a pre-processor transforms: ``@registry.register_transforms``
...
class Registry:
    ...
    def register_callback(cls, name):
        ...
        Usage::
            from src.common.registry import registry
            from src.trainers.callbacks.base import Callback
        ...
        def wrap(func):
            ...
            #assert issubclass(
            #    func, Callback
            #), "All callbacks must inherit the PyTorch Lightning Callback class"
    ...
    @classmethod
    def register_transforms(cls, name):
        r"""Register a processor to registry with key 'name'
        Args:
            name: Key with which the processor will be registered.
        Usage::
            from src.common.registry import registry
            from src.datasets.processors import BaseProcessor
    ...
    def register_datamodule(cls, name):
        ...
        Usage::
            from src.common.registry import registry
            import pytorch_lightning as pl
            @registry.register_datamodule("my_dataset")
        ...
        def wrap(datamodule_cls):
            ...
            ) or issubclass(datamodule_cls, torch.utils.data.dataset.Dataset), "All datamodules must inherit PyTorch or PyTorch Lightning DataModule class"

            cls.mapping["dataset_name_mapping"][name] = datamodule_cls
    ...
    def register_iteration_strategy(cls, name):
        ...
        Usage::
            ...
            from src.common.registry import registry
            from src.datasets.iterators import IterationStrategy
    ...
    def get(cls, name, default=None, no_warning=False):
        ...
        Args:
            ...
            no_warning (bool): If passed as True, warning when key doesn't exist
                               will not be generated. Useful for CoRL's
