...
def setup_imports():
    ...
    root_folder = registry.get("mae_stereo_root", no_warning=True)

    if root_folder is None:
        root_folder = os.path.dirname(os.path.abspath(__file__))
        root_folder = os.path.join(root_folder, "..")

        environment_mmf_path = os.environ.get("Masked-Auto-Encoder-for-Stereo-Depth-Estimation", os.environ.get("SRC_PATH"))
        
        if environment_mmf_path is not None:
            root_folder = environment_mmf_path
        
        registry.register("mae_stereo_path", root_folder)
